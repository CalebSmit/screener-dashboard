...
| Metric | Weight | What It Measures |
|--------|--------|-----------------|
| **FY1 EPS Revision (90d)** | 35% | Change in the consensus EPS estimate for the current fiscal year over the last 90 days, divided by the share price. Positive = analysts have raised their forecasts. Shown in basis points of price. |
...
**Why these?** The most important thing to understand about this category is the difference between a **revision** and a **surprise**. A surprise is backward-looking: the company already reported, and it beat or missed. A revision is forward-looking: analysts have changed their mind about the year ahead, and the change itself carries information before any results are published.

The academic evidence favours revisions clearly. Chan, Jegadeesh & Lakonishok (1996) tested all three legs of earnings momentum and found the analyst-revision measure the strongest, at a **+7.7% six-month spread** between the top and bottom deciles — replicated independently by Stickel (1991) at +7.07%. The surprise leg depends on *post-earnings announcement drift*, the tendency of prices to keep moving in the direction of a surprise for weeks afterwards, and Martineau (2022) documents that drift as **effectively gone since 2006** for everything except the smallest stocks. Since this screener covers only the S&P 500, that is precisely the population where the surprise signal is weakest. Practitioners agree: MSCI Barra's Sentiment factor is built from revision descriptors, and the widely-used Zacks Rank weights revision agreement and magnitude above surprise.

**This is why the weights changed on 2026-09-10.** Until then this category was named for revisions and contained none — 78 of its 100 points sat on the surprise family. It now leads with an actual revision metric. The category's overall 10% share of the final score did not change; only the split inside it did. Full reasoning and sources: `METHODOLOGY_CHANGELOG.md` 2026-09-10.

**One honest caveat, stated because it is a real cost.** Companies whose estimates are being revised upward also tend to be companies whose share price has already risen, so this metric overlaps with the Momentum category more than the others do (a correlation of about +0.42). That overlap was measured five different ways, including a version with no price in the formula at all, and it persists — so it reflects how markets actually behave rather than an artifact of the calculation. It is a genuine reduction in how independent the eight categories are, and it was accepted deliberately in exchange for the category measuring what its name says.

This category is weighted at only 10% because coverage can be sparse (not all stocks have active analyst coverage), and when coverage drops below usable levels, the weight automatically redistributes to the other categories.
...
5. **Limited revision history:** *(This limitation previously read "No EPS revision data … would require a paid data source like FactSet or Refinitiv I/B/E/S." That was **wrong**, and was corrected on 2026-09-10 when the metric shipped from the free data source already in use.)* The screener now carries a real consensus-revision signal, but only a shallow one: the data source exposes the consensus as it stood 7, 30, 60 and 90 days ago, so 90 days is the longest revision window available. A deeper history — needed to measure revision *persistence*, or to use the 6-month window Chan, Jegadeesh & Lakonishok (1996) tested — would still require a paid I/B/E/S feed.
