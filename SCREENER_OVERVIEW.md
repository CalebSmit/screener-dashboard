...
This is a quantitative stock screener. It takes every company in the S&P 500 (roughly 500 stocks), measures each one across up to 27 financial metrics, combines those measurements into a single composite score (0-100), and ranks the entire universe from best to worst. The top-ranked stocks form a model portfolio.

Not every stock sees all 27 metrics. The screener uses 23 generic metrics for most stocks and a separate set of 4 bank-specific metrics for financial companies (banks, insurers, credit companies). In practice, any individual stock is scored on about 23 metrics — the set just differs depending on whether the company is a bank or not.
...
| **FCF Yield** | 40% | Free cash flow (operating cash flow minus capital expenditures) divided by enterprise value. How much cash the business generates per dollar of total value. Higher = cheaper. |
...
| Metric | Weight | What It Measures |
|--------|--------|-----------------|
| **Earnings Yield** | 40% | Trailing EPS divided by share price (the inverse of the P/E ratio). Higher = cheaper. |
...
| Metric | Weight | What It Measures |
|--------|--------|-----------------|
| **Forward EPS Growth** | 35% | (Forward EPS - Trailing EPS) / Trailing EPS. Denominator floored at $1.00. Clamped to [-75%, +150%]. Higher = faster expected growth. |
| **PEG Ratio** | 20% | P/E ratio divided by forward EPS growth. A PEG of 1.0 means fairly valued relative to growth. Lower = better. |
| **Revenue Growth** | 30% | Year-over-year revenue increase from financial statements. Higher = growing top line. |
...
| **6-1 Month Return** | 50% | Total price return from 6 months ago to 1 month ago. Also skips the most recent month. |
...
| Metric | Weight | What It Measures |
|--------|--------|-----------------|
| **Volatility** | 60% | Annualized standard deviation of daily log returns over the past year. Lower = smoother ride. |
| **Beta** | 40% | Covariance of stock returns with S&P 500 returns divided by variance of market returns. Lower = less market-driven risk. |
...
| Metric | Weight | What It Measures |
|--------|--------|-----------------|
| **Analyst Surprise** | 40% | Median of (Actual - Estimated EPS) / max(|Estimated|, $0.10) over last 4 quarters. Positive = beat expectations. |
| **Price Target Upside** | 20% | (Mean Analyst Price Target - Current Price) / Current Price. Clamped to [-50%, +100%]. Higher = more analyst optimism. |
...
```
Valuation = 25% × EV/EBITDA_pct + 40% × FCF_Yield_pct + 20% × Earnings_Yield_pct + 15% × EV/Sales_pct
```
...
Every stock in the universe with all raw metrics, 8 category scores, the composite score, rank, value trap flag, growth trap flag, financial sector caveat flag, bank classification, and bank-specific metrics (P/B, ROE, ROA, Equity Ratio) where applicable. Each stock also carries a data provenance tag (`_data_source`), metric coverage count, and an EPS basis mismatch flag.

### Sheet 2: Screener Dashboard
The top 50 stocks, formatted for quick review. Includes rank, composite score, all 8 category scores, and the value trap and growth trap flags. Color-coded cells highlight strengths and weaknesses.
...
2. Computing up to 27 financial metrics across 8 categories (23 generic + 4 bank-specific, depending on company type)
