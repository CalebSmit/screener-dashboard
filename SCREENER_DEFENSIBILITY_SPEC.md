...
1. **Compute raw metrics** — the `METRIC_COLS` registry has **45 entries**: 29 scored generic + 4 bank-specific + 12 candidate metrics at weight 0 (pre-implemented but inactive until the self-improvement engine activates them). Non-bank stocks use the 29 generic metrics; bank-like stocks swap in the 4 bank-specific ones (P/B, ROE, ROA, Equity Ratio). *(Split corrected 2026-09-10; it had read 32/4/8, whose errors cancelled to the right total.)*
